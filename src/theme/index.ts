...
export function getDefaultState() {
	return {
		...
		// ADD OS: pinned to Light. Dark mode is out of scope for v1 (decision Q7).
		//
		// This was `useOsTheme().value === "dark" ? Dark : Light`. On a machine set to
		// dark, that booted the app into naive-ui's `darkTheme` — whose own defaults
		// fill in everything our light-only overrides do not — plus a `.theme-dark`
		// class on <html> that activates Tailwind's `dark:` variant and the template's
		// `.theme-dark` SCSS. The result was a half-themed dark mode, which is worse
		// than none. `useOsTheme` was imported for this line alone, so it is gone too.
		//
		// When a full dark palette lands in src/add-os/theme/tokens.ts, restore the
		// OS check here and flip DARK_MODE_SUPPORTED there.
		...
		rtl: true, // RTL (right to left) mode toggle — ADD OS: Arabic RTL is the base mode, not an option
		boxed: {
