	...
	<div class="page">
		...
		<div class="components-list">
			...
			<CardCodeExample title="Custom parsing">
				...
				<n-space vertical>
					<n-input-number :default-value="1075" :parse :format />
...
const CURRENCY_CHARS_REGEX = /([,$\s])/g

function parse(input: string) {
	const nums = input.replace(COMMA_REGEX, "").trim()
	if (NUMBER_INPUT_REGEX.test(nums)) return Number(nums)
	return nums === "" ? null : Number.NaN
}
...
function parseCurrency(input: string) {
	const nums = input.replace(CURRENCY_CHARS_REGEX, "").trim()
	if (NUMBER_INPUT_REGEX.test(nums)) return Number(nums)
	return nums === "" ? null : Number.NaN
}
