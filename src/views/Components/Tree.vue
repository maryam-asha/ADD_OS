	...
	<div class="page">
		...
		<div class="components-list">
			<CardCodeExample title="Basic">
				<n-tree block-line :data :default-expanded-keys selectable />
			...
			<CardCodeExample title="Search">
				<n-space vertical :size="12">
					...
					<n-tree :show-irrelevant-nodes :pattern :data="data1" block-line />
				</n-space>
				<template #code="{ html, js }">
					{{ html(`
					<n-space vertical :size="12">
...
import { NInput, NSpace, NSwitch, NTree } from "naive-ui"
import { ref } from "vue"
...
function createData(level = 4, baseKey = ""): TreeOption[] | undefined {
	if (!level) return undefined
	return [6 - level, undefined].map((_, index) => {
		const key = `${baseKey}${level}${index}`
		return {
			label: createLabel(level),
			key,
