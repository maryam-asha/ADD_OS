...
const tot = components.reduce((pre, cur) => pre + cur.items.length, 0)
	...
	return {
		label: () => (
			<div class="flex items-center justify-between gap-4">
				<div>Components</div>
				<NBadge value={tot} color="var(--border-color)" class="[&_.n-badge-sup]:text-tertiary!" />
			</div>
		),
		key: "components",
		...
		children: components.map(({ key, label, items }) => ({
			label: () => (
				<div class="flex items-center justify-between gap-4">
					<div>{label}</div>
					<NBadge value={items.length} color="var(--border-color)" class="[&_.n-badge-sup]:text-tertiary!" />
				</div>
			),
			key,
